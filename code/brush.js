...
tools.Linear = function(event, cx) {
  ...
  trackDrag(function(event) {
      ...
      const data = imageData.data;
      for (var i = 0; i < data.length; i += 4) {
          data[i] = r*mask[i/4]*paintFlowRate + (1-mask[i/4]*paintFlowRate)*data[i];
          data[i + 1] = g*mask[i/4]*paintFlowRate + (1-mask[i/4]*paintFlowRate)*data[i + 1];
          data[i + 2] = b*mask[i/4]*paintFlowRate + (1-mask[i/4]*paintFlowRate)*data[i + 2];
          data[i + 3] = 255;
      }
      cx.putImageData(imageData, currentPos.x - radius, currentPos.y - radius);
  });
...
tools.Gaussian = function(event, cx) {
  ...
  for(var y = 0; y < size; y++) {
    for (var x = 0; x < size; x++) {
      ...
      if (d > radius) d = radius;
      var sigma = 10;
      mask[i] = Math.exp(-0.5*(d/sigma)**2)/(sigma*Math.sqrt(2*Math.PI));
      i++;
    }
  }
  trackDrag(function(event) {
...
tools.Blur = function(event, cx) {
  ...
  for(var y = 0; y < size; y++) {
    for(var x = 0; x < size; x++) {
      ...
      filter[i] = Math.exp(-(d**2)/(2*sigma**2))/Math.sqrt(2*Math.PI*sigma**2);
      i++;
    }
  }
  //get sum
    ...
    function(event) {
      ...
      for(var x = xStart; x <= xEnd; x++) {
        for(var y = yStart; y <= yEnd; y++) {
          ...
          if(maskX < 0) { maskX = 0; } //bounds checking
          if(maskX >= cx.canvas.width) { maskX = cx.canvas.width - 1; }

          maskY = y + size/2; //start on top row of mask area
          if(maskY < 0) { maskY = 0; } //bounds checking
          if(maskY >= cx.canvas.height) { maskY = cx.canvas.height - 1; }
          ...
          for(var i = 0; i < data.length; i+=4) { //data length should be 3*3*4
      ...
      const tempData = tcx.getImageData(xStart, yStart, (xEnd - xStart), (yEnd - yStart));
